/*
 * bikeshare_network_steps_weighted_ids_nomax.cpp
 * 
 ...
 * 
 * different version: no max waiting time, trips are processed whenever it is possible
 * (waiting times are still minimized when performing matching, so hopefully waiting
 * times will have a limited variance)
 * 
 * 
...
struct bikeshare_calculate {
	protected:
		...
		struct trip_edge {
			...
			trip_edge() : first(0),second(0),dist(0) { } /* default constructor sets elements to avoid undefined behavior */
			trip_edge(unsigned int first_, unsigned int second_, unsigned int dist_):first(first_),second(second_),dist(dist_) { }
			bool read(read_table2& rt) {
				double dist_tmp;
				if(!rt.read(first,second,dist_tmp)) return false;
				dist = (unsigned int)round(dist_tmp);
				return true;
			}
			static const uint64_t magic_number = 7384202539752626841UL;
			
			operator matching_helper::edge() const { 
				return matching_helper::edge(first, second, dist);
			}
		};
		...
		struct node_edge {
			...
			bool read(read_table2& rt) {
				double dist_tmp;
				if(!rt.read(trip_id,node_id,dist_tmp)) return false;
				dist = (unsigned int)round(dist_tmp);
				return true;
			}
			static const uint64_t magic_number = 1854964730060634348UL;
		};
		...
		std::vector<trip_edge> trip_connections;
		std::vector<node_edge> node_connections; /* sorted by trip ID and travel time */
		
		/* trip starts, sorted by time */
		std::set<trip_event> start_events;
		std::unordered_map<unsigned int,const std::set<trip_event>::const_iterator> start_events_ids; /* index by event IDs (needed as well for trip graph iterator) */
	...
	public:
		explicit bikeshare_calculate() : t(0U), bikes_total(0U), bikes_counter(0U), trips_served(0U), time0(0U), max_vehicles(0U) { }
		...
		bool read_events(read_table2& rt) {
			...
			while(rt.read_line()) {
				...
				auto it = start_events.insert(trip_event{node1,dist_start,ts1,id});
				if(!it.second) throw std::runtime_error("bikeshare_calculate::read_events(): Error processing trip start!\n");
				start_events_ids.insert(std::pair<unsigned int, const std::set<trip_event>::const_iterator>(id,it.first));
			}
		...
		template<class It1, class Sent1>
		bool read_trip_connections(It1 it_trips, Sent1 st_trips, double speed,
				unsigned int max_wait_time, unsigned int max_reserve_time, bool precise_sort) {
			...
			for(;it_trips != st_trips;++it_trips) {
				trip_edge e = *it_trips;
				unsigned int travel_time = (unsigned int)ceil(e.dist / speed);
				if(travel_time > max_wait_time + max_reserve_time) continue;
				e.dist = travel_time;
				const trip_event& ee = trip_ends.at(e.first);
				const trip_event& se = *( start_events_ids.at(e.second) );
				if(ee.ts + travel_time <= se.ts + max_wait_time) trip_connections.push_back(e);
			}
			trip_connections.shrink_to_fit();
			...
			if(precise_sort) {
				...
				std::stable_sort(trip_connections.begin(),trip_connections.end(),[](const trip_edge& e1, const trip_edge& e2) {
				...
				while(i < trip_connections.size()) {
					...
					uint64_t target_node = start_events_ids.at(trip_connections[i].second)->n;
					for(j=i+1; j<trip_connections.size(); j++) {
						...
						if( start_events_ids.at(trip_connections[j].second)->n != target_node) break;
					}
					/* sort i -- j range by start_event */
					if(j > i+1) std::sort(trip_connections.begin() + i, trip_connections.begin() + j,[this](const trip_edge& e1, const trip_edge& e2) {
						const trip_event& t1 = *(start_events_ids.at(e1.second));
						const trip_event& t2 = *(start_events_ids.at(e2.second));
						return (t1 < t2);
					});
					i = j;
				}
			}
			else std::sort(trip_connections.begin(),trip_connections.end(),[](const trip_edge& e1, const trip_edge& e2) {
				if(e1.first < e2.first) return true;
				if(e1.first > e2.first) return false;
				return e1.dist < e2.dist;
			});
		...
		template<class It2, class Sent2>
		bool read_node_connections(It2 it_nodes, Sent2 st_nodes, double speed,
				unsigned int max_wait_time, unsigned int max_reserve_time) {
			node_connections.clear();
			/* 3. read connections to trip starts from nodes */
			for(;it_nodes != st_nodes;++it_nodes) {
				node_edge e = *it_nodes;
				unsigned int travel_time = (unsigned int)ceil(e.dist / speed);
				if(travel_time > max_wait_time + max_reserve_time) continue;
				e.dist = travel_time;
				node_connections.push_back(e);
			}
			node_connections.shrink_to_fit();
		...
		bool read_trip_connections(read_table2& rt_trips, double speed, unsigned int max_wait_time,
				unsigned int max_reserve_time, bool precise_sort = false) {
			return read_trip_connections(tsv_iterator<trip_edge>(rt_trips),tsv_iterator_sentinel(),speed,max_wait_time,max_reserve_time,precise_sort);
		}
		bool read_trip_connections(read_table2&& rt_trips, double speed, unsigned int max_wait_time,
				unsigned int max_reserve_time, bool precise_sort = false) {
			return read_trip_connections(tsv_iterator<trip_edge>(rt_trips),tsv_iterator_sentinel(),speed,max_wait_time,max_reserve_time,precise_sort);
		}
		bool read_node_connections(read_table2& rt_nodes, double speed, unsigned int max_wait_time,
				unsigned int max_reserve_time) {
			return read_node_connections(tsv_iterator<node_edge>(rt_nodes),tsv_iterator_sentinel(),speed,max_wait_time,max_reserve_time);
		}
		bool read_node_connections(read_table2&& rt_nodes, double speed, unsigned int max_wait_time,
				unsigned int max_reserve_time) {
			return read_node_connections(tsv_iterator<node_edge>(rt_nodes),tsv_iterator_sentinel(),speed,max_wait_time,max_reserve_time);
		...
		bool read_trip_connections(const connections_binary<trip_edge>& cb, double speed, unsigned int max_wait_time,
				unsigned int max_reserve_time, bool precise_sort = false) {
			return read_trip_connections(cb.begin(),cb.end(),speed,max_wait_time,max_reserve_time,precise_sort);
		}
		bool read_trip_connections(const char* binary_file, double speed, unsigned int max_wait_time,
				unsigned int max_reserve_time, bool precise_sort = false) {
			return read_trip_connections(connections_binary<trip_edge>(binary_file),speed,max_wait_time,max_reserve_time,precise_sort);
		}
		
		bool read_node_connections(const connections_binary<node_edge>& cb, double speed, unsigned int max_wait_time,
				unsigned int max_reserve_time) {
			return read_node_connections(cb.begin(),cb.end(),speed,max_wait_time,max_reserve_time);
		}
		bool read_node_connections(const char* binary_file, double speed, unsigned int max_wait_time,
				unsigned int max_reserve_time) {
			return read_node_connections(connections_binary<node_edge>(binary_file),speed,max_wait_time,max_reserve_time);
		}
		...
		void random_init(unsigned int nvehicles, R& rng) {
			...
			if(!start_events_ids.size()) throw std::runtime_error("bikeshare_calculate::random_init(): not trips present!\n");
			std::vector<unsigned int> trip_ids;
			trip_ids.reserve(start_events_ids.size());
			for(const auto& x : start_events_ids) trip_ids.push_back(x.first);
			std::uniform_int_distribution<size_t> dist(0UL,trip_ids.size()-1);
			for(unsigned int i=0;i<nvehicles;i++) {
				size_t x = dist(rng);
				uint64_t n = start_events_ids[ trip_ids[x] ]->n;
	...
	protected:
		...
		struct trips_graph_iterator {
			...
			trips_graph_iterator(const bikeshare_calculate& p_, unsigned int tmax_, unsigned int tmax_start_,
				...
				unsigned int max_connection_time_, unsigned int max_wait_time_, unsigned int max_start_time_)
					:p(p_),ix(p.end_events.cbegin()),tmax(tmax_),tmax_start(tmax_start_),exclude_matched(exclude_matched_),
					speed(speed_),max_connection_time(max_connection_time_),max_wait_time(max_wait_time_),max_start_time(max_start_time_) {
			...
			bool new_ix() {
				...
				auto it = std::lower_bound(p.trip_connections.cbegin(),p.trip_connections.cend(),ix->id,
					[](const trip_edge& e,unsigned int id) { return e.first < id; });
			...
			bool new_i() {
				for(;i<p.trip_connections.size() && p.trip_connections[i].first == ix->id;i++) {
					...
					const trip_event& e = *( p.start_events_ids.at(start_id) );
					if(e.ts <= tmax_start && t2 <= e.ts + max_wait_time && (max_start_time == 0 || t2 <= max_start_time)) {
						res.second = start_id;
						res.dist = 0;
		...
		struct trip_start_iterator {
			...
			std::set<trip_event>::const_iterator ix;
			size_t i;
			...
			unsigned int max_wait_time;
			bool is_end() const { return ix == p.start_events.cend() || ix->ts >= tmax; }
			bool operator == (const iterator_sentinel& dummy) const { return is_end(); }
			bool operator != (const iterator_sentinel& dummy) const { return !is_end(); }
			...
			trip_start_iterator(const bikeshare_calculate& p_, unsigned int tmax_, unsigned int ts_,
				unsigned int max_wait_time_, double speed_, const std::unordered_set<unsigned int>& exclude_matched_)
					:p(p_),exclude_matched(exclude_matched_),ix(p.start_events.cbegin()),
					speed(speed_),tmax(tmax_),ts(ts_),max_wait_time(max_wait_time_) {
				while ( ! (is_end() || new_ix()) ) ++ix;
			}
			...
			bool new_ix() {
				...
				auto it = std::lower_bound(p.node_connections.cbegin(), p.node_connections.cend(), ix->id,
		 ...
		 * result is the number of end events changed (either parked or adjusted to be processed later) */
		unsigned int process_end_events(unsigned int ts, double speed, FILE* fdetail) {
			unsigned int res = 0;
			for(auto it = end_events.begin(); it != end_events.end() && it->ts < t + ts; ) {
				unsigned int trip_id = it->id;
				if(matched_end.count(trip_id)) {
					it = end_events.erase(it);
					continue;
				}
				if(!matched_start.count(trip_id)) {
					++it;
					continue; /* just do nothing -- start event can still be matched later */
				}
				uint64_t n = it->n;
				...
				if(it2 != trip_end_delay.end()) {
					...
					res++; /* include this in result since it can be used next time */
					continue;
				}
	...
	public:
		 ...
		 * return false if there was no events processed or likely to be processed later */
		bool advance(unsigned int ts, bool treserve, unsigned int max_wait_time, double speed, FILE* fdetail, FILE* fout = 0, bool limit_new = false) {
			...
			bool ret = false;
			if(!treserve) ret = (process_end_events(ts,speed,fdetail) > 0);
				...
				unsigned int max_connection_time_, unsigned int max_wait_time_) */
			{
				...
				trips_graph_iterator it(*this, tmax_end, t+ts, matched_start, speed, 0, max_wait_time, t+2*ts);
				if(fout) fprintf(fout,"%u, calculating matching among trips, ",(unsigned int)time(0)-time0);
				matching_helper::calculate_matching(it, iterator_sentinel(), matching, max_wait_time, fout);
				if(matching.size()) ret = true;
				...
				for(auto x : matching) {
					...
					auto it = start_events_ids.find(start_id);
					if(it == start_events_ids.end()) throw std::runtime_error("bikeshare_calculate::advance(): missing start event!\n");
					start_events.erase(it->second);
					start_events_ids.erase(it);
			...
			{	
				...
				for(auto x : start_matching) {
					...
					auto it2 = start_events_ids.find(start_id);
					if(it2 == start_events_ids.end()) throw std::runtime_error("bikeshare_calculate::advance(): missing start event!\n");
					start_events.erase(it2->second);
					start_events_ids.erase(it2);
			...
			for(auto ix1 = start_events.begin(); ix1 != start_events.end() && ix1->ts < t + ts; ) {
			
				if(matched_start.count(ix1->id)) {
					start_events_ids.erase(ix1->id);
					ix1 = start_events.erase(ix1);
					continue;
				}
				
				uint64_t n = ix1->n;
				unsigned int start_id = ix1->id;
				//~ unsigned int t1 = ix1->ts;
				unsigned int delay = (unsigned int)ceil(ix1->node_dist / speed);
				if(treserve) {
					if(delay < ts) delay = 0;
					else delay -= ts;
				}
			...
			if(treserve) if(process_end_events(ts,speed,fdetail)) ret = true;
		...
		bool is_end() const {
			bool ret = true;
			if(!start_events.empty() && start_events.rbegin()->ts >= t) ret = false;
			if(!end_events.empty() && end_events.rbegin()->ts >= t) ret = false;
...
int main(int argc, char **argv)
{
	...
	bool trip_links_bin = false; /* if true, trip / node links input file is binary */
	bool node_links_bin = false;
	...
	if(!p.read_events(read_table2(trips,stdin))) {
		fprintf(stderr,"Error reading trip events!\n");
		return 1;
	}
	p.max_vehicles = max_vehicles;
	
	if(trip_links_bin) {
		if(!p.read_trip_connections(trip_links_fn,speed,max_wait_time,ts,precise_sort)) {
			fprintf(stderr,"Error reading trip link connections!\n");
			return 1;
		}
	}
	else {
		...
		else f_trip_links = fopen(trip_links_fn,"r");
		if(!f_trip_links) {
			fprintf(stderr,"Error opening input file %s!\n",trip_links_fn);
			return 1;
		}
		bool r1 = p.read_trip_connections(read_table2(f_trip_links),speed,max_wait_time,ts,precise_sort);
		if(links_zip) pclose(f_trip_links);
		else fclose(f_trip_links);
		if(!r1) {
			fprintf(stderr,"Error reading trip links!\n");
			return 1;
		}
	}
	
	if(node_links_bin) {
		if(!p.read_node_connections(node_links_fn,speed,max_wait_time,ts)) {
			fprintf(stderr,"Error reading node link connections!\n");
			return 1;
		}
	}
	else {
		FILE* f_node_links = 0;
		if(links_zip) {
			size_t len = strlen(node_links_fn);
			len += 16;
			char* tmp = (char*)malloc(sizeof(char)*len);
			if(!tmp) {
				fprintf(stderr,"Error allocating memory!\n");
		...
		else f_node_links = fopen(node_links_fn,"r");
		if(!f_node_links) {
			fprintf(stderr,"Error opening input file %s!\n",node_links_fn);
			return 1;
		}
		bool r1 = p.read_node_connections(read_table2(f_node_links),speed,max_wait_time,ts);
		if(links_zip) pclose(f_node_links);
		else fclose(f_node_links);
		if(!r1) {
			fprintf(stderr,"Error reading node links!\n");
			return 1;
		}
	}
	...
	if(random_init) p.random_init(random_init,rng);
	
	while(true) {
		bool cont = p.advance(ts,treserve,max_wait_time,speed,fdetail,stderr,limit_new);
		fflush(stderr);
		fprintf(stdout,"%u\t%u\t%u\n",p.get_ts(),p.get_num_vehicles(),p.get_trips_served());
		fflush(stdout);
		
		if(!cont && p.is_end()) break;
	}
